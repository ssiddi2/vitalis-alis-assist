  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('superbills').insert({
        ...
        cpt_lines: cptLines.map(l => ({ code: l.code, description: l.description, amount: Number(l.amount) })) as unknown as never,
        icd10_codes: icd10,
        insurance_snapshot: (insurance ?? null) as unknown as never,
        total_amount: total,
        status: 'generated',
