-- ============================================================
-- Final production-safety hardening: readiness wrapper, DoseSpot
-- partner-config parity, authoritative intake template selection.
-- Forward-only.
-- ============================================================

-- ---------- 0. Safe JSON helpers (never throw on malformed config) ----------
CREATE OR REPLACE FUNCTION public.jsonb_is_true(v jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path = '' AS $$
  SELECT v IS NOT NULL AND jsonb_typeof(v) = 'boolean' AND v = 'true'::jsonb;
$$;
REVOKE ALL ON FUNCTION public.jsonb_is_true(jsonb) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.jsonb_is_true(jsonb) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.jsonb_nonempty_text(v jsonb)
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path = '' AS $$
  SELECT v IS NOT NULL AND jsonb_typeof(v) = 'string' AND btrim(v #>> '{}') <> '';
$$;
REVOKE ALL ON FUNCTION public.jsonb_nonempty_text(jsonb) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.jsonb_nonempty_text(jsonb) TO authenticated, service_role;

-- ---------- 1. DoseSpot partner-config parity with _shared/dosespot.ts ----------
CREATE OR REPLACE FUNCTION public.dosespot_partner_config_blocker(pc jsonb, secret_refs text[])
RETURNS text LANGUAGE plpgsql IMMUTABLE SET search_path = '' AS $$
DECLARE
  pn jsonb; sg jsonb; ttl jsonb; ref text;
BEGIN
  IF pc IS NULL OR jsonb_typeof(pc) <> 'object' THEN RETURN 'partner_config_missing'; END IF;

  IF NOT public.jsonb_nonempty_text(pc -> 'host')
     OR (pc ->> 'host') !~ '^[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)+$' THEN
    RETURN 'partner_config_host_invalid';
  END IF;

  IF NOT public.jsonb_nonempty_text(pc -> 'launchPath') OR left(pc ->> 'launchPath', 1) <> '/' THEN
    RETURN 'partner_config_launch_path_invalid';
  END IF;

  pn := pc -> 'paramNames';
  IF pn IS NULL OR jsonb_typeof(pn) <> 'object'
     OR NOT public.jsonb_nonempty_text(pn -> 'clinicId')
     OR NOT public.jsonb_nonempty_text(pn -> 'clinicianId')
     OR NOT public.jsonb_nonempty_text(pn -> 'signature') THEN
    RETURN 'partner_config_param_names_invalid';
  END IF;

  ttl := pc -> 'launchTtlSeconds';
  IF ttl IS NULL OR jsonb_typeof(ttl) <> 'number'
     OR (ttl #>> '{}')::numeric < 1 OR (ttl #>> '{}')::numeric > 900
     OR (ttl #>> '{}')::numeric <> trunc((ttl #>> '{}')::numeric) THEN
    RETURN 'partner_config_launch_ttl_invalid';
  END IF;

  sg := pc -> 'signing';
  IF sg IS NULL OR jsonb_typeof(sg) <> 'object' THEN RETURN 'launch_signing_rule_not_configured'; END IF;
  IF NOT public.jsonb_nonempty_text(sg -> 'algorithm')
     OR lower(sg ->> 'algorithm') NOT IN ('hmac-sha256', 'hmac-sha512') THEN
    RETURN 'launch_signing_algorithm_unsupported';
  END IF;
  IF NOT public.jsonb_nonempty_text(sg -> 'encoding')
     OR (sg ->> 'encoding') NOT IN ('hex', 'base64') THEN
    RETURN 'launch_signing_encoding_unsupported';
  END IF;
  IF NOT public.jsonb_nonempty_text(sg -> 'secretRef') THEN
    RETURN 'launch_signing_secret_ref_missing';
  END IF;
  ref := btrim(sg ->> 'secretRef');
  IF secret_refs IS NULL OR NOT (ref = ANY (secret_refs)) THEN
    RETURN 'launch_signing_secret_ref_not_provisioned';
  END IF;

  IF NOT public.jsonb_nonempty_text(pc -> 'evidence_ref')
     OR NOT public.jsonb_is_true(pc -> 'evidence_approved') THEN
    RETURN 'partner_package_not_approved';
  END IF;

  IF coalesce(pc ->> 'controlled_substance_mode', '') NOT IN ('vendor_disabled', 'enabled') THEN
    RETURN 'controlled_substance_mode_unspecified';
  END IF;

  RETURN NULL;
END $$;
REVOKE ALL ON FUNCTION public.dosespot_partner_config_blocker(jsonb, text[]) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.dosespot_partner_config_blocker(jsonb, text[]) TO authenticated, service_role;

-- ---------- 2. Conservative intake template schema validator ----------
CREATE OR REPLACE FUNCTION public.obesity_intake_schema_blocker(content jsonb)
RETURNS text LANGUAGE plpgsql IMMUTABLE SET search_path = '' AS $$
DECLARE
  qs jsonb; q jsonb; ids text[] := '{}'; qid text; qtype text; aw jsonb; ref jsonb; reftype text; want text;
BEGIN
  ...
  FOR q IN SELECT x FROM jsonb_array_elements(qs) x LOOP
    ...
    IF qtype NOT IN ('boolean', 'choice', 'number', 'text') THEN RETURN 'template_question_type_invalid'; END IF;
    ...
    IF q ? 'red_flag_values' AND (jsonb_typeof(q -> 'red_flag_values') <> 'array'
        OR jsonb_array_length(q -> 'red_flag_values') = 0) THEN
      RETURN 'template_red_flag_values_invalid';
    END IF;
    IF qtype = 'choice' THEN
      IF NOT (q ? 'options') OR jsonb_typeof(q -> 'options') <> 'array'
         OR jsonb_array_length(q -> 'options') = 0 THEN
        RETURN 'template_options_invalid';
      END IF;
    ELSIF q ? 'options' AND jsonb_typeof(q -> 'options') <> 'array' THEN
      RETURN 'template_options_invalid';
    END IF;
  END LOOP;
  ...
  FOR q IN SELECT x FROM jsonb_array_elements(qs) x LOOP
    ...
    reftype := coalesce(ref ->> 'type', 'boolean');
    want := CASE reftype WHEN 'boolean' THEN 'boolean' WHEN 'number' THEN 'number' ELSE 'string' END;
    IF jsonb_typeof(aw -> 'equals') <> want THEN RETURN 'template_applies_when_type_mismatch'; END IF;
  END LOOP;
...
-- ---------- 3. Authoritative template selection for an encounter ----------
...
DECLARE
  e public.encounters; sl uuid; slcode text; t public.clinical_protocols;
BEGIN
  ...
  SELECT * INTO t FROM public.clinical_protocols c
   WHERE c.hospital_id = e.hospital_id AND c.kind = 'obesity_intake' AND c.status = 'approved'
     AND (c.service_line_id IS NULL OR c.service_line_id = sl)
     AND (c.state_code IS NULL OR c.state_code = e.patient_state_code)
...
CREATE OR REPLACE FUNCTION public.obesity_intake_template(p_encounter_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = '' AS $$
DECLARE e public.encounters; t public.clinical_protocols; blk text;
BEGIN
  IF auth.uid() IS NULL THEN RETURN jsonb_build_object('status','error','reason','not_authorized'); END IF;
  SELECT * INTO e FROM public.encounters WHERE id = p_encounter_id;
  ...
  RETURN jsonb_build_object('status','ok','id',t.id,'version',t.version,'title',t.title,
                            'questions', t.content -> 'questions');
END $$;
REVOKE ALL ON FUNCTION public.obesity_intake_template(uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.obesity_intake_template(uuid) TO authenticated, service_role;

-- ---------- 4. Intake submission uses the same selection + strict schema ----------
...
-- ---------- 5. DoseSpot readiness parity inside the go-live gate ----------
...
DECLARE
  ...
  p text; k text; ds public.vendor_onboarding; pc jsonb; pcblk text;
...
-- ---------- 6. Safe definer wrapper so clinicians can evaluate readiness ----------
CREATE OR REPLACE FUNCTION public.obesity_launch_readiness(
  p_hospital_id uuid, p_service_line_id uuid, p_state_code text,
  p_prescribing boolean DEFAULT true, p_controlled boolean DEFAULT false)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = '' AS $$
BEGIN
  IF auth.uid() IS NULL OR p_hospital_id IS NULL OR NOT public.is_my_hospital(p_hospital_id) THEN
    RETURN jsonb_build_object('authorized', false, 'can_launch', false,
      'blockers', to_jsonb(ARRAY['not_authorized']), 'gates', '[]'::jsonb);
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object('authorized', false, 'can_launch', false,
    'blockers', to_jsonb(ARRAY['readiness_unavailable']), 'gates', '[]'::jsonb);
END $$;
