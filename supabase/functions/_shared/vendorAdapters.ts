 ...
 * endpoint: the concrete request path must come from the vendor's own partner
 * package / OpenAPI schema and be stored on the profile's `capabilities.endpoints`.
 */

import { gatewayFetch, GatewayError, sha256Hex } from "./gateway.ts";
import { HEALTH_GORILLA_FHIR_BASE, resolveSecret, vendorGate, type OnboardingRow, type VendorKey } from "./vendors.ts";

export interface VendorOp {
  capability: string;
  /** Logical operation key resolved against `capabilities.endpoints`. */
  endpointKey: string;
  payload?: unknown;
...
export const stediAdapter: VendorAdapter = {
  ...
  async execute(row, op) {
    ...
    const url = endpointFor(row!, op.endpointKey);
    if (!url) return { status: "unavailable", reason: "endpoint_not_mapped_from_official_schema" };
    const apiKey = resolveSecret("STEDI_API_KEY_REF");
    if (!apiKey) return { status: "unavailable", reason: "secret_not_provisioned" };
    try {
      const res = await gatewayFetch({
        vendor: "stedi", env: row!.environment, url,
        headers: { Authorization: `Key ${apiKey}` },
        body: op.payload, idempotencyKey: op.idempotencyKey, correlationId: op.correlationId,
      });
      return {
...
async function healthGorillaToken(row: OnboardingRow): Promise<string | null> {
  ...
  const clientId = resolveSecret("HEALTH_GORILLA_CLIENT_ID_REF");
  const clientSecret = resolveSecret("HEALTH_GORILLA_CLIENT_SECRET_REF");
  const tokenUrl = resolveSecret("HEALTH_GORILLA_TOKEN_URL_REF");
  if (!clientId || !clientSecret || !tokenUrl) return null;

  const res = await gatewayFetch({
    vendor: "health_gorilla", env: row.environment, url: tokenUrl,
...
export const healthGorillaAdapter: VendorAdapter = {
  ...
  async execute(row, op) {
    const blocked = vendorGate(row, op.capability, row?.environment ?? "sandbox");
    if (blocked) return { status: "blocked", reason: blocked };
    const base = HEALTH_GORILLA_FHIR_BASE[row!.environment];
    const url = endpointFor(row!, op.endpointKey) ?? `${base}${op.endpointKey}`;
    const token = await healthGorillaToken(row!);
    if (!token) return { status: "unavailable", reason: "oauth_not_provisioned" };
    try {
      const res = await gatewayFetch({
        vendor: "health_gorilla", env: row!.environment, url,
        method: op.payload ? "POST" : "GET",
