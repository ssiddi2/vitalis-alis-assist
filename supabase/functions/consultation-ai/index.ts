  ...
  try {
    ...
    switch (action) {
      ...
      case "suggest_urgency": {
        if (!reason || !specialty) return jsonRes({ error: "Missing reason or specialty" }, 400);

        const out = await callAI([
          { role: "system", content: `You are a triage assistant. Given a consult specialty and clinical reason, output ONLY one word: "routine", "urgent", or "stat". Rules: stat = life/limb threat needing minutes; urgent = needs attention within hours; routine = within 24h. Output one word only.` },
          { role: "user", content: `Specialty: ${specialty}\nReason: ${reason}` },
        ]) as string;

        const normalized = (out || "").toLowerCase().match(/(stat|urgent|routine)/)?.[1] || "routine";
        return jsonRes({ urgency: normalized });
      }
